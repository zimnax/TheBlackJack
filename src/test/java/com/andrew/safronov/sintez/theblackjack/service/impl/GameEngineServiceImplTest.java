...
public class GameEngineServiceImplTest {
	...
	public void doHitCheckCardSum() {
		...
		savedDesk.getDealerCards().add(new Card(CardSuits.SPADES, CardRanks.FIVE));
		savedDesk.getDealerCards().add(new Card(CardSuits.DIAMONDS, CardRanks.FOUR));

		Desk doHitDesk = gameEngineServiceImpl.doHit(savedDesk.getPurse().getPurseId());

		Assert.assertEquals(19, doHitDesk.getPlayerPoints());
	}
}
